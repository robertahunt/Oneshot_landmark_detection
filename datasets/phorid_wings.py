...
class PhoridWings(data.Dataset):
    def __init__(
        ...
        assert not (
            ref_landmark is not None and prob_map is not None
    ...
    def prob_map_from_landmarks(self, size=(384, 384), kernel_size=192):
        ...
        prob_maps = get_guassian_heatmaps_from_ref(
            landmarks=landmarks,
        ...
        cv2.imwrite(
            f"imgshow/prob_map_ks{kernel_size}.jpg", (prob_map * 255).astype(np.uint8)
        )
        return prob_map
    ...
    def select_point_from_prob_map(self, prob_map, size=(192, 192)):
        ...
        if (
            prob_map[chosen_x1, chosen_y1] * np.random.random()
            > prob_map[chosen_x2, chosen_y2] * np.random.random()
        ):
    ...
    def retfunc1(self, index):
        ...
        raw_x = np.random.randint(
            int(pad_scale * self.size[0]), int((1 - pad_scale) * self.size[0])
        )
        raw_y = np.random.randint(
            int(pad_scale * self.size[1]), int((1 - pad_scale) * self.size[1])
        )
        # while True:
        ...
        temp = temp[3]
        chosen_y, chosen_x = (
            torch.div(temp.argmax(), patch_size, rounding_mode="floor"),
            temp.argmax() % patch_size,
        )
        if self.ret_name:
            return (
                item["image"],
    ...
    def retfunc_old(self, index):
        ...
        if self.transform != None:
            pth_img = os.path.join(self.pth_Image, item["ID"] + ".bmp")
            if self.pre_crop:
                item["image"] = self.transform(
                    self.pre_trans(Image.open(pth_img).convert("RGB"))
                )
            item["image"] = self.transform(Image.open(pth_img).convert("RGB"))
        ...
        if self.be_consistency:
            crop_imgs_aug = augment_patch(
                item["image"][
                    :,
                    margin_y : margin_y + patch_size,
                    margin_x : margin_x + patch_size,
                ],
                self.aug_transform,
            )
            temp = torch.zeros([1, patch_size, patch_size])
...
def test_Ph_img():
    ...
    for i in tqdm(range(2, 3)):
        ...
        vis2 = visualize(
            crop_imgs.unsqueeze(0), [[chosen_x, chosen_y]], [[chosen_x, chosen_y]]
        )
...
class Test_PhoridWings(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        for id, landmark in enumerate(landmark_list):
            ...
            template_patches[id] = item["image"][
                :, bottom : bottom + 192, left : left + 192
            ]
