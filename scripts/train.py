...
from datasets.phorid_wings import PhoridWings
...
def match_inner_product(feature, template):
    ...
    cos_similarity = torch.clamp(cos_similarity, 0.0, 1.0)
    assert (
        torch.max(cos_similarity) <= 1.0
    ), f"Maximum Error, Got max={torch.max(cos_similarity)}"
    assert (
        torch.min(cos_similarity) >= 0.0
    ), f"Maximum Error, Got max={torch.min(cos_similarity)}"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train Unet landmark detection network"
    )
    parser.add_argument("--tag", default="", help="name of the run")
    parser.add_argument(
        "--config", default="configs/config.yaml", help="default configs"
    )
    ...
    assert config["dataset"]["name"] in ["ceph", "phorid_wings"]

    # Tester
    tester = Tester(logger, config, tag=args.tag)

    if config["dataset"]["name"] == "ceph":
    ...
    elif config["dataset"]["name"] == "phorid_wings":
        dataset = PhoridWings(
            config["dataset"]["pth"],
            "Train",
            patch_size=32 * 8,
    ...
    if args.pretrain is not None and args.pretrain != "":
        ...
        ckpt2 = (
            ...
            + "emb-16"
            + f"/model_patch_epoch_{epoch}.pth"
        )
    ...
    if config["training"].get("cuda", False):
        net = net.cuda()
    ...
    for epoch in range(config["training"]["num_epochs"]):
        ...
        for index, (raw_img, crop_imgs, chosen_y, chosen_x, raw_y, raw_x) in enumerate(
            dataloader
        ):
            with torch.autograd.set_detect_anomaly(False):
                ...
                tmpl_feature = torch.stack(
                    [
                        crop_fea_list[0][[id], :, tmpl_y[id], tmpl_x[id]]
                        for id in range(gt_y.shape[0])
                    ]
                ).squeeze()
                ret_inner_5 = match_inner_product(
                    raw_fea_list[0], tmpl_feature
                )  # shape [8,12,12]
                ...
                tmpl_feature = torch.stack(
                    [
                        crop_fea_list[1][[id], :, tmpl_y[id], tmpl_x[id]]
                        for id in range(gt_y.shape[0])
                    ]
                ).squeeze()
                ret_inner_4 = match_inner_product(raw_fea_list[1], tmpl_feature)
                loss_4 = ce_loss(
                    ret_inner_4, gt_y, gt_x, nearby=config["training"]["nearby"]
                )
                ...
                tmpl_feature = torch.stack(
                    [
                        crop_fea_list[2][[id], :, tmpl_y[id], tmpl_x[id]]
                        for id in range(gt_y.shape[0])
                    ]
                ).squeeze()
                ret_inner_3 = match_inner_product(raw_fea_list[2], tmpl_feature)
                loss_3 = ce_loss(
                    ret_inner_3, gt_y, gt_x, nearby=config["training"]["nearby"]
                )
                ...
                tmpl_feature = torch.stack(
                    [
                        crop_fea_list[3][[id], :, tmpl_y[id], tmpl_x[id]]
                        for id in range(gt_y.shape[0])
                    ]
                ).squeeze()
                ret_inner_2 = match_inner_product(raw_fea_list[3], tmpl_feature)
                loss_2 = ce_loss(
                    ret_inner_2, gt_y, gt_x, nearby=config["training"]["nearby"]
                )
                ...
                tmpl_feature = torch.stack(
                    [
                        crop_fea_list[4][[id], :, tmpl_y[id], tmpl_x[id]]
                        for id in range(gt_y.shape[0])
                    ]
                ).squeeze()
                ret_inner_1 = match_inner_product(raw_fea_list[4], tmpl_feature)
                loss_1 = ce_loss(
                    ret_inner_1, gt_y, gt_x, nearby=config["training"]["nearby"]
                )
        ...
        logger.info(
            "Epoch {} Training logic loss 5 {:.3f} 4 {:.3f} 3 {:.3f} 2 {:.3f} 1 {:.3f}".format(
                epoch,
                losses[0].mean(),
                losses[1].mean(),
                losses[2].mean(),
        ...
        if (epoch) % config["training"]["save_seq"] == 0:
            ...
            torch.save(
                net_patch.state_dict(),
                runs_dir + "/model_patch_epoch_{}.pth".format(epoch),
            )
