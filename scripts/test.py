...
from datasets.phorid_wings import Test_PhoridWings
...
class Tester(object):
    def __init__(self, logger, config, net=None, tag=None, train="", args=None):
        ...
        if config["dataset"]["name"] == "ceph":
            ...
            dataset_1 = Test_PhoridWings(config["dataset"]["pth"], "Train")
    ...
    def test(
        self, net, epoch=None, dump_label=False, tag=None, oneshot_id=1, draw=True
    ):
        if net is not None:
            self.model = net
        ...
        for i in range(1):
            if config["dataset"]["name"] == "ceph":
                one_shot_loader = Test_Cephalometric(
                    pathDataset=self.config["dataset"]["pth"],
                    mode="Oneshot",
                    id_oneshot=oneshot_id,
                )
            elif config["dataset"]["name"] == "phorid_wings":
                one_shot_loader = Test_PhoridWings(
                    pathDataset=self.config["dataset"]["pth"],
                    mode="Oneshot",
                    id_oneshot=oneshot_id,
                )
            ...
            for id_depth in range(6):
                ...
                for id_mark, landmark in enumerate(landmarks_ori):
                    tmpl_y, tmpl_x = landmark[1] // (2 ** (5 - id_depth)), landmark[
                        0
                    ] // (2 ** (5 - id_depth))
                    mark_feature = features_tmp[id_depth][0, :, tmpl_y, tmpl_x]
                    tmp.append(mark_feature.detach().squeeze().cpu().numpy())
                tmp = np.stack(tmp)
                one_shot_feature = torch.tensor(tmp)
                if config["training"].get("cuda", False):
                    one_shot_feature = one_shot_feature.cuda()
            ...
            for img, landmark_list in self.dataloader_1:
                ...
                if draw:
                    ...
                    image_pred.save(
                        tfilename(
                            config["base"]["runs_dir"], "visuals", str(ID), "pred.png"
                        )
                    )

                if dump_label:
                    inference_marks = {
                        id: [int(preds[1][id]), int(preds[0][id])] for id in range(32)
                    }
                    dir_pth = tfilename(
                        config["base"]["runs_dir"],
                        f"oneshot_id_1",
                        "pseudo-labels_init",
                    )
                    with open(
                        tfilename("{0}/{1:03d}.json".format(dir_pth, ID)), "w"
                    ) as f:
...
if __name__ == "__main__":
    ...
    net = UNet_Pretrained(
        3, non_local=config["training"]["non_local"], emb_len=16, conf_m=True
    )
    ...
    if args.pretrain is not None and args.pretrain != "":
        ...
        ckpt = (
            "/data/quanquan/oneshot/runs-pixelpro/" + tag + f"/model_epoch_{epoch}.pth"
        )
        ...
        ckpt2 = (
            "/data/quanquan/oneshot/runs-pixelpro/"
            + tag
    ...
    for i in range(100):
        mre = tester.test(
            net, net_patch, epoch=epoch, dump_label=False, oneshot_id=i + 1, draw=False
        )
        mre_list.append(mre)
